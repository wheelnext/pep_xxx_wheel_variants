...
#### 2.1 Without any Variant Provider - PIP should install a CPU only build
...
> [!NOTE]  
> In the case of PyTorch, the `Null-Variant` is a *CPU-only* PyTorch build [could be changed at anytime - just a decision for this demo].
>
> ```bash
> ############################## Variant: `00000000` #############################
> ################################################################################
...
NVIDIA CUDA 12.8 is installed on the machine, which explains why the NVIDIA CUDA 11.8 build of PyTorch was rejected.
...
>
> ```bash
> Would install ... nvidia-cublas-12.8.3.14-aa00cb06 ... nvidia-nvtx-12.8.55-aa00cb06
> ```
>
> Which indicates that the right variant of NVIDIA CUDA Libraries was selected and installed (they only need major level)
>
> ```bash
> ############################## Variant: `aa00cb06` #############################
